...
export default function WorkshopApplyPage({ searchParams }: { searchParams?: { kind?: string } }) {
  const [busy, setBusy] = useState(false);
  const [done, setDone] = useState(false);
  const [kind, setKind] = useState(searchParams?.kind || "dj");
  ...
  return (
    <Section title="Workshop Başvuru" description="Kısa formu doldur, seni arayalım.">
      <form onSubmit={submit} className="grid gap-4 max-w-2xl mx-auto">
        ...
        {kind === "dj" ? (
          <div className="grid gap-3 rounded-xl border border-white/10 p-3">
            <div className="text-sm text-white/80">DJ Workshop Soruları</div>
            <input name="q_deneyim" className="rounded-xl bg-black border border-white/20 px-3 py-2 text-white" placeholder="Deneyim (yıl)" />
            <input name="q_ekipman" className="rounded-xl bg-black border border-white/20 px-3 py-2 text-white" placeholder="Ekipman" />
            <input name="q_tarz" className="rounded-xl bg-black border border-white/20 px-3 py-2 text-white" placeholder="Tarzlar" />
          </div>
        ) : (
          <div className="grid gap-3 rounded-xl border border-white/10 p-3">
